...
def construct_bound_mats_ineq(n_obs, x0_idx, m_idx):
    ...
    # 3a. this stands for P_mus(k+1) - P_mus(k) <= 0  k=1,2,...,m-1
    # which is converted to -P_mus(k+1) + P_mus(k) >= 0, because
    # fmin_slsq does not utilize negative inequalities
    for i in range(0, m_idx):
        bm_ineq[i][i+2] = 1
        bm_ineq[i][i+3] = -1

    # 3b. this stands for P_mus(k+1) - P_mus(k) >= 0  k=m,m+1,...,q-1
...
def construct_bound_mats(n_obs, x0_idx, m_idx):
    ...
    equalities defined in 3a, 3b, and 3c.
    """
...
def perform_constrained_optimization(flow, vols, pressure, x0, m_idx, r_max=100, r_min=0, e_max=100, e_min=0, p_min=-15, p_max=20, initial_guess=None):
    ...
    result = fmin_slsqp(obj, initial_guess, eqcons=[eq_con], ieqcons=[ieq_con], bounds=bounds)
    R = result[0]
    E = result[1]
    P_mus = result[2:]
